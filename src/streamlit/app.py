...
def clean_data(self) -> pd.DataFrame:
    ...
    return self.data
            ...
            st.markdown(
                "> Plotting our absolute correlations to see which variables with which correlations can we plot."
            )
            st.pyplot(vc.corrs_abs_plot)
            st.markdown(
                "***We see that variables with absolute correlation greater than ```0.2``` is apt for plotting.***"
            )
            st.text("\n")
            ...
            st.markdown(
                "### ***Given some data, this detector says if it significantly deviates from the norm.***"
            )
